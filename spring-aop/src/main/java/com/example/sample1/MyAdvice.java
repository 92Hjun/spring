...
// @Aspect 어느테이션은 이 클래스가 AOP 적용에 필요한 설정이 정의된 클래스임을 나타낸다.
@Aspect
public class MyAdvice {
   ...
   public void loggingException(JoinPoint jp, Exception ex)throws Exception{
	   ...
	   // 발생한 예외를 HTAException으로 변환해서 발생시킨다.
	   throw new HTAException(errorMsg);
   }
   ...
   public void logging(JoinPoint jp) {
      ...
      for (Object arg : args) {
    	  // 매개변수의 타입이름 조회하기
         String argTypeName = arg.getClass().getName();
         
         if (count > 0) {
            argTypeBuilder.append(", ");
         }
         // 조회된 매개변수의 타입을 argTypeBuilder에 추가하기
         argTypeBuilder.append(argTypeName + " arg" + ++count);
         // 조회된 매개변수의 값을 argValueBuilder에 추가하기
      ...
      // 대상객체의 클래스타입을 조회하기
      Class<?> clazz = jp.getTarget().getClass();
      // 대상객체의 클래스타입으로 Logger객체 얻기
      Logger logger = LoggerFactory.getLogger(clazz);
      // 로그 출력하기
      logger.info(message.toString(), methodName, argTypeBuilder.toString(), argValueBuilder.toString());
   }
