			...
			<div class="col-sm-12">
				...
				<c:if test="${pagination.totalRows ne 0 }">
					<c:if test="${pagination.beginPage ne pagination.currentBlock }">
						<li><a href="prosendmessagebox?pno=${pagination.beginPage-1 }"><span aria-hidden="true">&laquo;</span></a></li>
					</c:if>
					<c:forEach begin="${pagination.beginPage }" end="${pagination.endPage }" var="pageNo">
						<li><a href="prosendmessagebox?pno=${pageNo }">${pageNo }</a></li>
					</c:forEach>
					<c:if test="${pagination.totalBlocks ne pagination.currentBlock }">
						<li><a href="prosendmessagebox?pno=${pagination.endPage+1 }"><span aria-hidden="true">&raquo;</span></a></li>
					</c:if>
				</c:if>
				</ul>
			</div>
