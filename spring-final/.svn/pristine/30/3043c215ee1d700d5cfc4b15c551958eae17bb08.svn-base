...
<%@ include file="/WEB-INF/views/navi/sidebarstud.jsp" %>
	<div class="container" style="margin-left: 250px; padding-top:25px;">
		...
		<div class="text-right">
			...
			<a href="stufreeboard" class="btn btn-default btn-xs">뒤로가기</a>
		</div>
		<hr>
		<div class="row">
			...
			<c:if test="${!empty reviewList }">
				<c:forEach var="review" items="${reviewList }">
					<div class="well">
						...
						<p>${review.contents }</p>
						<div class="text-right">
							<a href="deletefreeboardreview?rno=${review.no }" class="btn btn-danger btn-xs">삭제</a>
						</div>
					</div>
					<hr>
				</c:forEach>
		...
		<div class="row well">
			<form action="freeboardaddreview?bno=${board.no }" method="post">
